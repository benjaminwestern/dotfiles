...
resolve_linux_plan() {
  ...
  RESOLVED_PRESERVE_PATHS="${CLI_PRESERVE_PATHS:-$(state_get PRESERVE_PATHS)}"
  [[ "$RESOLVED_PRESERVE_PATHS" != __CLEAR__ ]] || RESOLVED_PRESERVE_PATHS=''
  export RESOLVED_PROFILE RESOLVED_SHELL RESOLVED_ZSCALER RESOLVED_DOTFILES RESOLVED_PACKAGES RESOLVED_APPLICATIONS RESOLVED_MISE_TOOLS RESOLVED_SHELL_DEFAULT RESOLVED_CODE_DIRECTORY RESOLVED_DOWNLOADS_LINK RESOLVED_DOWNLOADS_TARGET RESOLVED_GIT_IDENTITY RESOLVED_LINUX_DEFAULTS RESOLVED_REMOTE_ACCESS RESOLVED_LINUX_HOSTNAME RESOLVED_LINUX_DEFAULT_APPS RESOLVED_DEVICE_NAME RESOLVED_GIT_USER_NAME RESOLVED_GIT_USER_EMAIL RESOLVED_PRESERVE_PATHS
}
...
path_preserved() {
  ...
  done < <(printf '%s' "${RESOLVED_PRESERVE_PATHS:-}" | tr '|' '\n')
  return 1
}
