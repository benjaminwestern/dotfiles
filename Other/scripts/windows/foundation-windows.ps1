...
function Ensure-Profile {
  ...
  $blockContent = @(
    ...
    '}'
    ''
    '# Bypass cmd.exe for interactive npm CLIs so Ctrl+C cannot leave a batch prompt behind.'
    'function global:Invoke-MisePowerShellTool {'
    '  param([Parameter(Mandatory)][string]$Name, [object[]]$Arguments)'
    '  $resolved = (& mise which $Name 2>$null | Select-Object -First 1)'
    '  if (-not $resolved) { throw "mise could not resolve ''$Name''." }'
    '  if ($resolved -match ''\.cmd$'') {'
    '    $launcher = $resolved -replace ''\.cmd$'', ''.ps1'''
    '  } else {'
    '    $launcher = @("$resolved.ps1", "$resolved-ps.ps1") | Where-Object { Test-Path $_ } | Select-Object -First 1'
    '  }'
    '  if ($launcher -and (Test-Path $launcher)) { & $launcher @Arguments; return }'
    '  & $resolved @Arguments'
    '}'
    'function global:opencode { Invoke-MisePowerShellTool -Name ''opencode'' -Arguments $args }'
    'function global:opencode2 { Invoke-MisePowerShellTool -Name ''opencode2'' -Arguments $args }'
    'function global:codex { Invoke-MisePowerShellTool -Name ''codex'' -Arguments $args }'
    ''
